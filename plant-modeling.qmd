...
```{r}
ggplot(data = la_red, aes(x = avg_area, y = pop_trend)) + 
  geom_jitter(width = 0, height = 0.05, alpha = 0.8) 
```

```{r}
ggplot(data = la_red, aes(x = avg_area, y = pop_trend_binary)) + 
  geom_jitter(width = 0, height = 0.05, alpha = 0.8) +
...
```{r}
mod_photo <- glm(pop_trend_binary~avg_area, data = la_red, family = 'binomial') # binomial dist; prob of 1 and 0 over many trials
summary(mod_photo)
```
